...
module.exports = new function() {
	...
	this.stringFormatDateTime = function(testRun) {
		...
		if(Ti.Platform.osname === 'tizen' || Ti.Platform.osname === 'mobileweb'){
			valueOf(testRun, String.formatDate(d, 'long')).shouldBe('Friday, February 01, 2013'); //Android\iPhone - 'November 01, 2013'
			valueOf(testRun, String.formatDate(d, 'short')).shouldBe('2/1/2013'); //Android\iPhone - 02/01/13

			valueOf(testRun, String.formatTime(d, 'long')).shouldBe('1:02:03 AM'); // on Android\Iphone - no spaces! - '1:02:03pm'
			valueOf(testRun, String.formatTime(d, 'short')).shouldBe('1:02 AM'); // on Android\Iphone - no spaces! - '1:02pm'
		}else{
