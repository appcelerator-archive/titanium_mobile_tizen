...
module.exports = new function() {
	...
	this.basic_picker = function(testRun) {
        ...
        this.get_set_row = function(testRun) {
            ...
            valueOf(testRun, picker.type).shouldBeEqual(Titanium.UI.PICKER_TYPE_PLAIN); //type
            
            var data = [];
            
            valueOf(testRun, function(){
                data[0]=Ti.UI.createPickerRow({title:first_text, color: '#00FF00'});//Fisrt green
            }).shouldNotThrowException();
            
            valueOf(testRun, function(){
                data[1]=Ti.UI.createPickerRow({title:second_text, color: '#0000FF'});//Second blue
            }).shouldNotThrowException();
        ...
        this.time_picker = function(testRun) {
            ...
            if(Ti.Platform.osname === 'tizen' || Ti.Platform.osname === 'mobileweb') {
                ...
                var greenPixelsCB = function(count){
                    valueOf(testRun, count).shouldBeGreaterThan(1000);
                    win.close();
                    finish(testRun);
                }
        ...
        this.column = function(testRun) {
            ...
            //START check column properties and methods__________________________________________
        ...
        this.row = function(testRun) {
            ...
            //START Check all properties and methods for row_________________________
