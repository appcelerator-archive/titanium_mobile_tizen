...
module.exports = new function() {
	...
	var MIN_WINDOW_PERCENTAGE = 80;
	var FULL_SCREEN_PERCENTAGE = 100;
	...
	this.pagingControlTimeout = function(testRun) { 
		...
		var testedEnv = new TestedEnv(true,5000,  function(){testedEnv.scrollableView.currentPage = 1});
	...
	this.addView = function(testRun) {
		...
		function checkViewsAddYellow(){
			var viewYellow = Ti.UI.createView({ backgroundColor: YELLOW_RGB});	
			valueOf(testRun, testedEnv.scrollableView.views.length).shouldBeEqual(3);
		...
		var testedEnv = new TestedEnv(false, 10, checkViewsAddYellow);		
