...
module.exports = new function() {
    ...
    this.successDownloadFlowTest = function(testRun) {
        ...
        var urlDownload = new tizen.URLDownload("http://download.tizen.org/sdk/InstallManager/tizen-sdk-2.0-ubuntu32.bin", "wgt-private-tmp", "tmp"+(new Date().getTime()));
    ...
    this.failedDownloadTest = function(testRun) {
        ...
        // Start downloading large file to be able to test callbacks.
        var urlDownload = new tizen.URLDownload("http://download.tizen.org/Magic-Sofware-Package-v4.2.bin", "wgt-private-tmp", "tmp"+(new Date().getTime()));
        var downloadId = tizen.download.start(urlDownload, listener);
        valueOf(testRun, downloadId).shouldBeGreaterThanEqual(0); //
    }
    ...
    this.successDownloadTest = function(testRun) {
        ...
        var listener = {
            ...
            onpaused: function(id) {
                Ti.API.debug("'onpaused' event. id=" +id);
                valueOf(localTestRun, "onpaused").shouldBe("oncompleted"); //failed!
                finish(localTestRun);
            },
            onaborted: function(id) {
                Ti.API.debug("'onaborted' event. id=" +id);
                valueOf(localTestRun, "onaborted").shouldBe("oncompleted"); //failed!
                finish(localTestRun);
            },
            oncompleted: function(id, fileName) {
                Ti.API.debug("'oncompleted' event. id=" +id+", fileName=" + fileName);
        ...
        // Start downloading large file to be able to test callbacks.
        var urlDownload = new tizen.URLDownload("http://download.tizen.org/sdk/1_0-larkspur/pkg_list_windows", "wgt-private-tmp", "tmp"+(new Date().getTime()));
        var downloadId = tizen.download.start(urlDownload, listener);
        valueOf(testRun, downloadId).shouldBeGreaterThanEqual(0); //
    }
}
