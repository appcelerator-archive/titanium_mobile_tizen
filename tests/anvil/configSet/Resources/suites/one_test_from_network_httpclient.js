...
module.exports = new function() {
	...
	this.name = "one_test_from_network_httpclient";
	this.tests = [
		//{name: "apiTest"},
	...
	this.responseHeadersBug = function(testRun) {
		...
		xhr.onload = function(e) {
			...
			Ti.API.info("This function tests only android platform");
	...
	this.clearCookiePositiveTest = function(testRun) {
		...
		var callback_error = function(e){
			Ti.API.info("clearCookiePositiveTest; callback_error");
			Ti.API.debug(e);
			valueOf(testRun, true).shouldBeFalse();
		};
		var timer = 0;
		var second_cookie_fn = function(e) {
			Ti.API.info("clearCookiePositiveTest Start: from second_cookie_fn:");
			var second_cookie_string = this.getResponseHeader('Set-Cookie').split(';')[0];
			Ti.API.info(second_cookie_string);
			Ti.API.info("clearCookiePositiveTest Start => clearTimeout");
			try {
				clearTimeout(timer);
		...
		var done = false;
		var cookie_string;
		var timer_id = xhr.setTimeout(30000);
		Ti.API.info("clearCookiePositiveTest; timer_id: "+timer_id);
		xhr.onload = function(e) {
			Ti.API.info("clearCookiePositiveTest; xhr.onload");
			cookie_string = this.getResponseHeader('Set-Cookie').split(';')[0];
			Ti.API.info("clearCookiePositiveTest; cookie_string: "+ cookie_string);
		...
		xhr.onerror = function(e) {
			Ti.API.info("clearCookiePositiveTest; xhr.onerror");
			clearTimeout(timer);
			callback_error(e);
		};
		xhr.open('GET','https://my.appcelerator.com/auth/login');
	...
	this.clearCookieUnaffectedCheck = function(testRun) {
		...
		var callback_error = function(e){
			Ti.API.info("clearCookieUnaffectedCheck; callback_error");
			Ti.API.debug(e);
			valueOf(testRun, true).shouldBeFalse();
		};
		var timer = 0;
		var second_cookie_fn = function(e) {
			...
			Ti.API.info("clearCookieUnaffectedCheck Start: from second_cookie_fn:");
			var second_cookie_string = this.getResponseHeader('Set-Cookie').split(';')[0];
			Ti.API.info(second_cookie_string);
			Ti.API.info("clearCookieUnaffectedCheck Start => clearTimeout");
		...
		xhr.onload = function(e) {
			...
			Ti.API.info("clearCookieUnaffectedCheck => onload; cookie_string: "+ cookie_string);
