		...
		function foldersListCB(folders) {
			...
			function syncFolder(item) {
				function folderSynced() {
					...
					var emailFolderMessages = require("ui/handheld/tizen/platform/messaging/emailFolderMessages");
					
					// Open selected folder
					args.containingTab.open(new emailFolderMessages({emailService: emailService, folderName: item.rowData.title, folderId: folders[item.index].id, containingTab: args.containingTab}));
				}
