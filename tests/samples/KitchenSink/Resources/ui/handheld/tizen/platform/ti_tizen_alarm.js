function tizen_alarm() {
	...
	function addAbsoluteAlarm(time, period){
		var period = Math.floor(period),
		    alarm = Ti.Tizen.createAlarmAbsolute(time, period);
	...
	function addRelativeAlarm(delay, period){
		...
		var alarm = Ti.Tizen.createAlarmRelative(delay, period);
	...
	function createRow(alarm){
		...
		if( alarm instanceof tizen.AlarmAbsolute || alarm instanceof Ti.Tizen.AlarmAbsolute){
