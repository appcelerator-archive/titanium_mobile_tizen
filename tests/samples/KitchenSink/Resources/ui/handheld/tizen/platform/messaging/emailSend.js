function emailSend(args) {
	...
	function initEmailService(callBack) {
		Ti.API.info("Start to find email service.");
	...
	addDraftEmailBtn.addEventListener("click", function(e) {
		function addDraftMessage() {
			...
			try {
				Ti.API.info("Start to add draft email message.");
				
				// Add new draft email message
				checkMessageData() && emailService.messageStorage.addDraftMessage(new tizen.Message(serviceType, {subject: subjectField.value, plainBody: textArea.value, to: [emailField.value]}), draftMessageAdded, errorCB);
			} catch (exc){
	...
	sendEmailBtn.addEventListener("click", function(e) {
		function sendNewEmail() {
			function emailSent(recipients) {
				Ti.API.info("Email sent successfully to " + recipients.length + " recipient(s).");
				Ti.UI.createAlertDialog({
					title: "Info",
