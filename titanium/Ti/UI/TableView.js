...
define(['Ti/_/declare', 'Ti/_/UI/KineticScrollView', 'Ti/_/style', 'Ti/_/lang', 'Ti/UI/MobileWeb/TableViewSeparatorStyle', 'Ti/UI'],
	...
	return declare('Ti.UI.TableView', KineticScrollView, {
		...
		_insertRow: function(value, index, isInsertAfter) {
			var location = this._calculateLocation(index);
			if (location) {
                            location.section.add(value, isInsertAfter ? location.localIndex+1 : location.localIndex); // We call the normal .add() method to hook into the sections proper add mechanism
			}
			this._publish(value);
		...
		insertRowAfter: function(index, value) {
			this._insertRow(value, index, true/*After*/);
		},
			...
			data: {
				set: function(value) {
					if (is(value,'Array')) {
						...
						// Convert any object literals to either TableViewRow or TableViewSection instances
						for (i in value) {
							if (!isDef(value[i].declaredClass) || (value[i].declaredClass != "Ti.UI.TableViewRow" && value[i].declaredClass != "Ti.UI.TableViewSection")) {
								//check header or footer

								//temporary fix for Tizen KitchenSink problem:
								//Properties 'header' and 'footer' are not correct
								//So we copy these properties to 'headerTitle' and 'footerTitle'
								if (isDef(value[i].header)) {
									value[i].headerTitle = value[i].header; 
								}
								if (isDef(value[i].footer)) {
									value[i].footerTitle = value[i].footer; 
								}
								
								
								if (isDef(value[i].headerTitle) || isDef(value[i].footerTitle)) {
									//add section
									var section = UI.createTableViewSection(value[i]);
									section._tableView = this;
									//if 'title' exists then add row in section
									isDef(value[i].title) && section.add(UI.createTableViewRow(value[i]));
									value[i] = section;
								} else {
									//add row
