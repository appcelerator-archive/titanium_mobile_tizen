exports.TitaniumInterface = (function(){
        ...
        if(list.length>0) {
            for(var k = 0, l = list.length; k<l; k++) {
                switch(list[k].type) {
                    case 'operation':
                        result+='       ';
                        //resultInside+= (list[k].idlType == 'void')?'_':'';
                        result+= list[k].name + ': function(';
                        //putting arguments for methods 
                        var arg = list[k].arguments;
                        var type = 'sync ';
                        var tmpRes = '';
                        for(var h=0; h<arg.length; h++) {
                            tmpRes+=arg[h].name + '/*'+arg[h].type.idlType+'*/';
                            tmpRes+=(arg[h+1])?', ':'';
                            
                            if(arg[h].name.indexOf('Callback') != -1 || arg[h].type.idlType.indexOf('Callback') != -1) {
                                type = 'async'; 
                                methods.type  = 'async';
                            }
                        } 
                        ...
                        result+='){\n';
                        methods.add(type+'---');
                        methods.add(list[k].name + '(');
                        methods.add(tmpRes);
                        methods.add(')\n');
                        
                        result+=(!list[k+1] && lastComma)?'       }\n':'       },\n';
                        //result+='       },\n';
                        break;
                    case 'attribute':
                        result+='       ' + list[k].name + ': '+ checkValue(list[k].idlType); 
                        result+=(!list[k+1] && lastComma)? '//' + list[k].idlType.idlType + '\n':',' + '//' + list[k].idlType.idlType + '\n';
                        //result+='       ,' + '//' + list[k].idlType.idlType + '\n';
                        break;
                    case 'const':
    ...
    var result = {
        ...
        getConstants: function(){
            ...
            var constants = [];
            for(var i=0, len = this.dA.length; i<len; i++) {
                if(this.dA[i] && this.dA[i].type == 'enum') {
                    function createST(string) {
                        function isUpperCase(myString) { 
                            return (myString == myString.toUpperCase()); 
                        } 
                        var arr = string.split('');
                        for(var p=0, q = arr.length; p<q; p++) {
                            if(isUpperCase(arr[p]) && p!=0) {
                                arr[p]='_'+arr[p];
                            }
                        }
                        var r = arr.join('');
                    ...
                    var list = this.dA[i].values;
                    for(var n=0, m=list.length; n<m; n++) {
                        var v = [];
                        v[0] = createST(this.dA[i].name).toUpperCase().replace(".","_").replace("-","_")+'_'+list[n].toUpperCase().replace(".","_").replace("-","_");  
                        v[1] = n;
                        constants.push(v);
                    }
                }
            ...
            (function(){
                if(constants.length>0) {
                    ...
                    for(var i=0, len=constants.length; i<len; i++) {
                        view+='             '+constants[i][0]+' :'+constants[i][1];
                        view+=(i==len-1)?'\n':',\n';
                    }
                    view+='       },\n';
                }
                        ...
                        switch(this.dA[i].extAttrs[q].name) {
                            case 'Constructor':
                                for(var z = 0, x = this.dA[i].extAttrs[q].arguments.length; z < x; z++) {
                                    cP += this.dA[i].extAttrs[q].arguments[z].name +'/*'+this.dA[i].extAttrs[q].arguments[z].type.idlType+'*/';
                                    cP += (q == s-1 && z == x-1) ? '' : ', ';
                                }
                                if(q == s-1) { // this check because interfase can has many Constructors, but js need only one
                                    view+= '       create'+this.dA[i].name+': function('+ cP +'){\n';
                                    
                                    view+= '            return new (require("'+options.titaniumFolder+this.folderName+'/'+this.dA[i].name+'"))('+ cP +');\n'
                                    view+= (!this.dA[i+1])?'       }\n':'       },\n';
